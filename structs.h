...
struct Agent
{
    double start_i, start_j, goal_i, goal_j, length;
...
class Vector2D {
  public:
    ...
    inline void operator -=(const Vector2D &vec) { i -= vec.i; j -= vec.j; }
    inline bool operator ==(const Vector2D &vec) { return i == vec.i && j == vec.j; }
    inline bool operator !=(const Vector2D &vec) { return i != vec.i && j != vec.j; }
};
