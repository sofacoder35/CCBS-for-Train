...
Config::Config()
{
    ...
    cbs_mode = CN_CBS_MODE;
    alarmMark = CN_ALARMMARK;
}
...
void Config::getConfig(const char *fileName)
{
    ...
    element = algorithm->FirstChildElement("cbs_mode");
    if (!element)
    {
        std::cout << "Error! No 'cbs_mode' element found inside '"<<CNS_TAG_ALGORITHM<<"' section. It's compared to '"<<CN_CBS_MODE<<"'."<<std::endl;
        cbs_mode = CN_CBS_MODE;
    }
    else
    {
        ...
        stream<<value;
        stream>>cbs_mode;
        if(cbs_mode < 0 || cbs_mode > 4)
        {
            std::cout << "Error! Wrong 'cbs_mode' value found inside '"<<CNS_TAG_ALGORITHM<<"' section. It's compared to '"<<CN_CBS_MODE<<"'."<<std::endl;
            cbs_mode = CN_CBS_MODE;
        }
        stream.clear();
        stream.str("");
    ...
    element = algorithm->FirstChildElement("alarmMark");
    if (!element)
    {
        std::cout << "Error! No 'alarmMark' element found inside '"<<CNS_TAG_ALGORITHM<<"' section. It's compared to '"<<CN_ALARMMARK<<"'."<<std::endl;
        cbs_mode = CN_CBS_MODE;
    }
    else
    {
        ...
        if(alarmMark < 0.0 )
        {
            std::cout << "Error! Wrong 'alarmMark' value found inside '"<<CNS_TAG_ALGORITHM<<"' section. It's compared to '"<<CN_ALARMMARK<<"'."<<std::endl;
            alarmMark = CN_ALARMMARK;
        }
        stream.clear();
        stream.str("");
